...
if [ -n "$PIDS" ]
then
  echo "Clojure-rest-example is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/clojure-rest-example.out
ERR_FILE=${LOGGING_PATH}/clojure-rest-example.err
...
echo "Giving Clojure-rest-example $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Clojure-rest-example didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Clojure-rest-example failed"
else
  echo "Starting Clojure-rest-example succeeded"
fi
